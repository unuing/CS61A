...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        for _ in range(self.min_range):
            ...
            if place is beehive:
                break
        while place is not beehive and dist <= self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        for bee in self.place.bees[:]:
            bee.reduce_armor(amount if self.armor > amount else amount + self.damage)
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        self.true = QueenAnt.is_first_queen
        QueenAnt.is_first_queen = False
        # END Problem 13
    ...
    def action(self, gamestate):
        ...
        if self.true:
            ...
            p = self.place.exit
            while p is not None:
                if p.ant is not None and p.ant not in QueenAnt.buffed_ants:
                    p.ant.damage *= 2
                    QueenAnt.buffed_ants.append(p.ant)
                if hasattr(p.ant, "contained_ant") and p.ant.contained_ant is not None and p.ant.contained_ant not in QueenAnt.buffed_ants:
                    p.ant.contained_ant.damage *= 2
                    QueenAnt.buffed_ants.append(p.ant.contained_ant)
                p = p.exit
        else:
...
class LaserAnt(ThrowerAnt):
        ...
        while place is not beehive:
            ...
            if place.ant is not None and place.ant is not self:
                ...
                if isinstance(place.ant, ContainerAnt) and place.ant.contained_ant is not None:
                    insects[place.ant.contained_ant] = dist
            place = place.entrance
            dist += 1
        return insects
        # END Problem OPTIONAL
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem OPTIONAL
        return max(2 - self.insects_shot*0.05 - distance*0.2, 0)
        # END Problem OPTIONAL
