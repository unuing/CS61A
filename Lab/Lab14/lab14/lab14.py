def prune_min(t):
    ...
    if t.is_leaf():
        return
    t.branches.remove(max(t.branches, key=lambda b: b.label))
...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        if skel_char == code_char:
            match_edits, match_cost = helper_align(skeleton_idx + 1, code_idx + 1)
            match_edits = code_char + match_edits
            possibilities.append((match_edits, match_cost))
        # Insert
...
def num_splits(s, d):
    ...
    count = 0
    sum_s = sum(s)
    def helper(s, sum_sofar):
        nonlocal count
        if len(s) == 0:
            if abs(sum_sofar - (sum_s - sum_sofar)) <= d:
                count += 1
            return
        helper(s[1:], sum_sofar + s[0])
        helper(s[1:], sum_sofar)
    helper(s, 0)
    return count // 2
