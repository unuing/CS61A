...
def interleaved_sum(n, odd_term, even_term):
    ...
    True
    """
    def helper(start, odd_term, even_term):
        if start > n:
            return 0
        return odd_term(start) + helper(start + 1, even_term, odd_term)
...
def mutate_reverse(link):
    ...
    """
    reversed_link = Link.empty
    it = link
    while it is not Link.empty:
        reversed_link = Link(it.first, reversed_link)
        it = it.rest
    it, rit = link, reversed_link
    while it is not Link.empty:
        it.first = rit.first
        it, rit = it.rest, rit.rest
