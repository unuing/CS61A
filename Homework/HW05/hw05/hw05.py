...
def make_withdraw(balance, password):
    ...
    """
    incorrect_passwords = []
    def withdraw(amount, passwd_entered):
        nonlocal balance
        if len(incorrect_passwords) >= 3:
            return 'Too many incorrect attempts. Attempts: {0}'.format(incorrect_passwords)
        elif passwd_entered == password:
            if amount <= balance:
                balance -= amount
                return balance
            else:
            ...
            incorrect_passwords.append(passwd_entered)
...
def merge(incr_a, incr_b):
    ...
    while next_a != float('inf') or next_b != float('inf'):
        if next_a == next_b:
            yield next_a
            next_a, next_b = next(iter_a, float('inf')), next(iter_b, float('inf'))
        elif next_a < next_b:
            yield next_a
...
def make_joint(withdraw, old_pass, new_pass):
    ...
    "Too many incorrect attempts. Attempts: ['my', 'secret', 'password']"
    """
    def joint_withdraw(amount, passwd_entered):
        if passwd_entered == new_pass:
            return withdraw(amount, old_pass)
        else:
            return withdraw(amount, passwd_entered)
    
    check_old_password = withdraw(0, old_pass)
...
def remainders_generator(m):
    ...
    11
    """
    def generate(remainder):
        i = remainder if remainder else m
        while True:
            yield i
            i += m
