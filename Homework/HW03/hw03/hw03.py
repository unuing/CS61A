...
def count_change(total):
    ...
    True
    """
    def max_coin(power = 0):
        if 2 ** (power + 1) > total:
            return 2 ** power
        else:
            return max_coin(power + 1)
    
    def count(tot = total, maximum_coin = max_coin()):
        if 0 <= tot < 2:
            return 1
        if maximum_coin == 0 or tot < 0:
            return 0
        return count(tot - maximum_coin, maximum_coin) + count(tot, maximum_coin // 2)
